...
const total_diffs = 4 #Total number of difference in this level
...
#Sets the text to 0/total number of difference when the scene is first loaded.
func _ready():
...
#A function that checks if a difference was found or not, and increases HP by 1. To make up for the function above (_on_Area2D5_input_event) reducing HP by 1 when finding a difference.
